        ...
        with st.spinner("Processando arquivos..."):
            try:
                ...
                if not df_contabil_limpo.empty:
                    ...
                    df_contabil_limpo.dropna(subset=['Conta_Chave'], inplace=True)
                    df_contabil_limpo['Conta_Chave'] = df_contabil_limpo['Conta_Chave'].astype('int64')
                    st.dataframe(df_contabil_limpo)
                    st.session_state['contabil_ok'] = True
                else:
                    st.error("Não foi possível processar o relatório contábil.")
                    st.session_state['contabil_ok'] = False
                ...
                if not extratos_encontrados:
                    ...
                    st.session_state['extrato_ok'] = False
                else:
                    ...
                    df_extrato_unificado.dropna(subset=['Conta_Chave'], inplace=True)
                    df_extrato_unificado['Conta_Chave'] = df_extrato_unificado['Conta_Chave'].astype('int64')
                    st.dataframe(df_extrato_unificado)
                    st.session_state['extrato_ok'] = True
