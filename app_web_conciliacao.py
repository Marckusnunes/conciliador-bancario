...
if 'df_resultado' in st.session_state:
    ...
    if isinstance(resultado, pd.DataFrame):
        ...
        # Seção de auditoria restaurada
        st.markdown("---")
        with st.expander("Clique aqui para auditar os dados de origem"):
            st.subheader("Dados Extraídos do Relatório Contábil (Pronto para Conciliação)")
            ...
            if ('audit_contabil' in st.session_state and st.session_state['audit_contabil'] is not None and
                ...
                if 'Conta_Chave' in df_c.columns and 'Conta_Chave' in df_e.columns:
                    chaves_comuns = pd.merge(
                        df_c[['Conta_Chave', 'Conta', 'Titular']].rename(columns={'Conta': 'Conta_Contabil', 'Titular': 'Titular_Contabil'}),
                        df_e[['Conta_Chave', 'Conta', 'Titular']].rename(columns={'Conta': 'Conta_Extrato', 'Titular': 'Titular_Extrato'}),
                        on='Conta_Chave',
                        how='inner'
                    )
